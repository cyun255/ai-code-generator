...
public class FileDirReadTool extends BaseTool {
    ...
    public String readDir(@P("目录的相对路径，为空则读取整个项目结构") String relativeDirPath,
        ...
        try {
            ...
            if (!path.isAbsolute()) {
                ...
                Path projectRoot = Paths.get(AppConstant.CODE_GEN_ROOT, projectDirName);
            ...
            // 使用 Hutool 递归获取所有文件
            List<File> allFiles = FileUtil.loopFiles(targetDir, file -> !shouldIgnore(file.getPath()));
            // 按路径深度和名称排序显示
            allFiles.stream()
                    .sorted((f1, f2) -> {
    ...
    private boolean shouldIgnore(String fileName) {
        // 检查是否在忽略名称列表中
        for (String extension : IGNORED_NAMES) {
