...
class AiCodeGeneratorFacadeTest {
    ...
    void generateAndSaveMultiCode() {
        ...
        Flux<String> result = aiCodeGeneratorFacade.generateAndSaveCode(message, CodeGenTypeEnum.MULTI_FILE, 1L);
    ...
    void generateAndSaveSingleHtmlCode() {
        String message = "给我生成一份登陆页面，不超过20行代码";
        Flux<String> result = aiCodeGeneratorFacade.generateAndSaveCode(message, CodeGenTypeEnum.SINGLE_HTML, 2L);
        List<String> list = result.collectList().block();
        Assertions.assertNotNull(list);
        System.out.println(String.join("", list));
    }
